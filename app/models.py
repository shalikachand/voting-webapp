...
class NomineeBridge(db.Model):
    ...
    sid = db.Column(db.Integer, db.ForeignKey('InAppResponse.id'),
                    primary_key=True)
...
class Nominee(db.Model):
    ...
    year_level = db.Column(db.Integer, db.ForeignKey('YearLevel.id'),
                           nullable=False)
    photo = db.Column(db.String)
    division = db.Column(db.Integer, db.ForeignKey('Division.id'))

    year_level_rel = db.relationship("YearLevel",
                                     backref=db.backref("nominees", lazy=True))
